...
{
  if (narg < 4 || (narg-4) % 2) error->all(FLERR,"Illegal compute radf command");

  array_flag = 1;
  extarray = 0;

  nbin = force->inumeric(FLERR,arg[3]);
  if (nbin < 1) error->all(FLERR,"Illegal compute radf command");
  if (narg == 4) npairs = 1;
  ...
  memory->create(hist,npairs,nbin,"radf:hist");
  memory->create(histall,npairs,nbin,"radf:histall");
  memory->create(array,nbin,1+2*npairs,"radf:array");
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (ipair)
        for (ihisto = 0; ihisto < ipair; ihisto++)
          hist[radfpair[ihisto][itype][jtype]][ibin] += 1.0;
      if (newton_pair || j < nlocal) {
        if (jpair)
          for (ihisto = 0; ihisto < jpair; ihisto++)
            hist[radfpair[ihisto][jtype][itype]][ibin] += 1.0;
      }
    }
  }
