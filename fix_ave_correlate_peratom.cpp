...
memory->grow(indices_group,ngroup_loc,"ave/correlate/peratomindices_group");
  ...
  for (i = 0; i < nvalues; i++) {
    ...
    group_data_loc[a][i] = data;
    if ( (dynamics == ORTHOGONAL || dynamics == ORTHOGONALSECOND) && nsample < nsave) {
      group_data_loc[a][i+nvalues+6] = data;
    }
      }
    }
  ...
  if (variable_flag == VAR_DEPENDENED){
    ...
    group_data_loc[a][nvalues+include_orthogonal] = peratom_data[indices_group[a]];
      }
    }
    memory->destroy(peratom_data);
    ...
    if(memory_switch==PERATOM){
      ...
      group_data_loc[a][nvalues+include_orthogonal+r] = x[indices_group[a]][r];
    }
      }
    }
  }
  ...
  if (dynamics == ORTHOGONAL || dynamics == ORTHOGONALSECOND) {
    ...
    if(memory_switch==PERATOM){
      ...
      group_data_loc[a][r+nvalues] = v[indices_group[a]][r];
    }
      }
    }
    for (a= 0; a < ngroup_loc; a++) {
    ...
    if(memory_switch==PERATOM){
      ...
      group_data_loc[a][r+nvalues+3] = f[indices_group[a]][r];
    }
      }
    }
  }
  ...
  if(memory_switch==GLOBAL){
    ...
    //load proc_number
    int p;
    int *proc_number,*proc_number_red,*proc_ind;
    if (me != 0)
      MPI_Send(&ngroup_loc,1,MPI_INT,0,0,MPI_COMM_WORLD);
    else {
memory->create(proc_number,nprocs-1,"ave/correlate/peratom:proc_number");
      for ( p=1; p<nprocs; p++)
	MPI_Recv(&proc_number[p-1],1,MPI_INT,p,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      //printf("ME: proc_number = %d\n",ngroup_loc);
      //for ( p=1; p<nprocs; p++)
        //printf("from PROC %d: proc_number = %d\n",p,proc_number[p-1]);
    }

    int *indices_group_trans;
memory->create(indices_group_trans,ngroup_loc,"ave/correlate/peratom:indices_group_trans");
    for ( a=0; a<ngroup_loc; a++) {
      tagint *ids_ptr;
      ids_ptr = std::find(group_ids,group_ids+ngroup_glo,tag[indices_group[a]]);
      int ind = (ids_ptr - group_ids);
      indices_group_trans[a] = ind;
    }
    //get particle information
    if (me != 0){
MPI_Send(indices_group_trans,ngroup_loc,MPI_INT,0,0,MPI_COMM_WORLD);
    }
    else {
memory->create(proc_number_red,nprocs-1,"ave/correlate/peratom:proc_number_red");
memory->create(proc_ind,ngroup_glo-ngroup_loc,"ave/correlate/peratom:proc_ind");
      proc_number_red[0] = 0;
      for (p=1; p<nprocs-1; p++) proc_number_red[p] = proc_number_red[p-1] + proc_number[p-1];
      for ( p=1; p<nprocs; p++)
 MPI_Recv(&proc_ind[proc_number_red[p-1]],proc_number[p-1],MPI_INT,p,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      //for ( a=0; a<ngroup_loc; a++)
    //printf("ind = %d\n",indices_group[a]);
      //for ( a=0; a<ngroup_glo-ngroup_loc; a++)
    //printf("ind = %d\n",proc_ind[a]);
    }

    //gather data and store it
    if (me != 0)
      MPI_Send(group_data_loc,ngroup_loc*(nvalues + include_orthogonal+variable_nvalues),MPI_DOUBLE,0,0,MPI_COMM_WORLD);
    else {
      //write own data
      for (a=0; a < ngroup_loc; a++)
    for (i=0; i<nvalues + include_orthogonal+variable_nvalues; i++) {
      group_data[indices_group_trans[a]][i]=group_data_loc[a][i];
    }
      //gather proc data
      double **proc_data;
      memory->create(proc_data,ngroup_glo-ngroup_loc,nvalues + include_orthogonal+variable_nvalues,"ave/correlate/peratom:proc_data");
      for ( p=1; p<nprocs; p++)
 MPI_Recv(&proc_data[proc_number_red[p-1]][0],proc_number[p-1]*(nvalues + include_orthogonal+variable_nvalues),MPI_DOUBLE,p,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
      for (a=0; a<ngroup_glo-ngroup_loc; a++)
    for (i=0; i<nvalues + include_orthogonal+variable_nvalues; i++)
      group_data[proc_ind[a]][i]=proc_data[a][i];
    }

    //write data in full array
    if (me == 0) {
      for (a= 0; a < ngroup_glo; a++) {
